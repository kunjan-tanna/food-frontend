...
import StartersPro from "./startersPro";
import ChinesePro from "./chinesePro";
import DessertsPro from "./dessertsPro";
import SoupsPro from "./soupsPro";
...
import { saveOrderIn } from "../../redux/actions/order";
...
class Menu extends React.Component {
   ...
   /* handle the OrderIn from Child Component*/
   handleOrder = (del) => {
      this.props.dispatch(saveOrderIn(del));
   };
   ...
   render() {
      const banID =
         this.state.rowData && this.state.rowData.filter((i) => i.banquetId);
      return (
         <Row>
            <Col md="6" sm="12">
               {this.state.rowData &&
                  this.state.rowData.map((i, index) => {
                     if (i.banName === "Punjabi") {
                        return (
            ...
            <Col md="6" sm="12">
               <Cart
                  orderIn={this.handleOrder}
                  value={this.props.orderIn}
                  styleTable={styles.table}
...
const mapDispatchToProps = (dispatch) => {
   return {
      removeall: () => dispatch(globalActions.removeItem()),
      removeItem: (data) => dispatch(globalActions.removeParticularItem(data)),
      dispatch,
   };
};
